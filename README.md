...
```python
# Dropping NaN values that occur as a result of calculating pct change for returns
...
```python
# Plotting data with a secondary axis since close column and return have different measurements.
df_dji.plot(figsize = (20, 12), secondary_y = "Return", mark_right = True, fontsize = 15)
plt.show()
```
...
df_dji.rename(columns = {"Close":"DJI_Close", "Return":"DJI_Return"}, inplace = True)
df_dji
```
...
df_dji["position"] = np.sign(df_dji["DJI_Return"])
df_dji
```
...
```python
# Adding our strategy return to the dataset
df_dji["Strategy_Ret"] = df_dji["position"].shift() * df_dji["DJI_Return"]
df_dji
```
...
```python
# Adding a cumulative product
df_dji.Strategy_Ret.add(1, fill_value = 0).cumprod()
df_dji["Strategy"] = df_dji.Strategy_Ret.add(1, fill_value = 0).cumprod()*df_dji.iloc[0,0]
df_dji
```
...
```python
# Plotting DJI and our strategy to visualize the performances
df_dji[["DJI_Close", "Strategy"]].plot(figsize = (15,10), fontsize =15)
...
```python
# We have 252 trading days so we will make a fuctionto cal return and risk
...
```python
# Call the function
summary_ann(df_dji[["DJI_Return", "Strategy_Ret"]])
```
